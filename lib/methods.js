...
Meteor.methods({
	'buy'(code, orderData) {
		if (orderData && orderData.name) {
			const order = Recipes.findOne({name: orderData.name});
			if (order) {
				let customer;
				if (Meteor.isServer) {
				...
				if (customer) {
					Customers.update({ code: customer.code }, { $inc: { total: order.price } })
					Orders.insert({ code: customer.code, name: customer.name, table: customer.table, item: order.name, title: order.title, price: order.price, createdAt: new Date(), status: order.status[0], quantity: 1 });
				...
				throw new Meteor.Error('pedido-nao-encontrado', `Pedido não encontrado: ${orderData}`);
			}
		}
	},
	'waiter'(code) {
	...
	'addItem'({ code, name, table, item, price, quantity }) {
		if (item && quantity > 0) {
			const objItem = Recipes.findOne({name: item });
	...
	'moveOrder'(_id, direction) {
		...
		if (order) {
			...
			const item = Recipes.findOne({ name: order.item })
			if (item) {
